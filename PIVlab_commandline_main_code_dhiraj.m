...
directory= sprintf('/Users/dhirajksimngh/Desktop/snowpixel/TIV_2019_5_april/Travis_data', trials(trial)); %directory containing the images you want to analyze
...
time = 16; %Enter the real time in ms
...
% s{4,1}= 'Mask';                  s{4,2}=[];         % If needed, generate via: imagesc(image); [temp,Mask{1,1},Mask{1,2}]=roipoly;
 s{5,1}= 'ROI';                   s{5,2}=[140,74,924-140,746-74]; % Region of interest: [x,y,width,height] in pixels, may be left empty                                    s{5,2}=[1,1,260,140]; 
...
%     image2 = PIVlab_preproc (image2,p{1,2},p{2,2},p{3,2},p{4,2},p{5,2},p{6,2},p{7,2},p{8,2});
    [x{counter}, y{counter}, u{counter}, v{counter}, typevector{counter}] = piv_FFTmulti (image1,image2,s{1,2},s{2,2},s{3,2},s{5,2},s{6,2},s{7,2},s{8,2},s{9,2},s{10,2});
    clc
...
for PIVresult=1:size(x,1)
    ...
    typevector_filt{PIVresult,1}=typevector_filtered;
      ...
      h(i) =  quiver(u_filtered,v_filtered,'k','AutoScaleFactor', 1.5);
...
% filename = sprintf('/Users/apple/Documents/MATLAB/dhiraj/image.mat',trials(trial));
