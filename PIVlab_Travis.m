...
save_path = '/Users/travismorrison/Desktop/TIV'; 
directory = '/Users/travismorrison/Desktop/TIV'; %directory containing the images you want to analyze
...
realdist = 50000; % Enter the real distance in mm- total length of image
dist = 1024; % Enter the number of pixel
time = 8; %Enter the real time in ms betwen images
...
% s{4,1}= 'Mask';                  s{4,2}=[];         % If needed, generate via: imagesc(image); [temp,Mask{1,1},Mask{1,2}]=roipoly;
 s{5,1}= 'ROI';                   s{5,2}=[139,70,951-139,783-70]; % Region of interest: [x,y,width,height] in pixels, may be left empty
...
%% PIV analysis loop
if mod(amount,2) == 1 %Uneven number of images?-> beacuse ¥ou need pairs to perform the piv
...
%     image2 = PIVlab_preproc (image2,p{1,2},p{2,2},p{3,2},p{4,2},p{5,2},p{6,2},p{7,2},p{8,2});
    [x{counter}, y{counter}, u{counter}, v{counter}, typevector{counter}] = piv_FFTmulti (image1,image2,s{1,2},s{2,2},s{3,2},s{4,1},s{5,2},s{6,2},s{7,2},s{8,2},s{9,2},s{10,2});
    clc
...
%     hold on;
%           quiver(x{counter},y{counter},u{counter},v{counter},'k','AutoScaleFactor', 3);
%         quiver(u{counter},v{counter},'k','AutoScaleFactor', 1.5);
%     hold off;
%     axis image;
...
umin = 0; % minimum allowed u velocity
umax = 5; % maximum allowed u velocity
vmin = 0; % minimum allowed v velocity
...
figure()
quiver(u_filtered,-v_filtered,'k','AutoScaleFactor', 1.5);
saveas(gcf, strcat(save_path,'/test_TIV',piv), 'jpg')
axis tight
end
...
%  imshow(h)
%         saveas(gcf,sprintf('H_%2d.tiff',i+1),'tiffn'); 
% %   save('u_11.txt', 'u_filtered','-ASCII')
%  save('v_11.txt', 'v_filtered','-ASCII')
% % Post processing ends here
%}
%% Saving the data
% Saving in common location
%  filename = sprintf('/Users/apple/Desktop/wind_snow_2019/output.mat',trials(trial));
%  save (filename,'x','y','u_filt','v_filt');

% Saving in respective folder
% mkdir (sprintf('/Users/apple/Documents/MATLAB/dhiraj/image/output',trials(trial)));
% filename = sprintf('/Users/apple/Documents/MATLAB/dhiraj/image/output.mat',trials(trial));
...
%    hold off;
%    axis off;
%    save_name = sprintf('G:/Vortex Ring Experiments/Polymer_soln_expts/PAMH_5ppm/Global/Ensemble/L100_V100/Trial_%01d/PIV_auto_output/vorticity/vort_filt%04d.bmp',trials(trial),i);
%    saveas(im_vorticity(i), save_name);
% end

% for i=1:(amount/2)
% %     im_u_vel(i) = imagesc((u_filt{i},4,'cubic'),[-0.5 0.5]);
% %    colorbar;
% %    hold on;
% %    quiver(x{i},y{i},u_filt{i}*scale_vector,v_filt{i}*scale_vector,'linewidth',0.5,'color','k','autoscale', 'off');
%  h(i)=quiver(u_filtered(i),v_filtered(i),'k','AutoScaleFactor', 1.5);
%    hold off;
%    axis off;
%    save_name = sprintf('/Users/apple/Desktop/TIV_IWC/piv/1%04d.bmp',trials(trial),i);
%    
%    saveas(h(i), save_name);
% end
...
% end
%}
clearvars -except p s x y u v vort typevector directory filenames u_filtered v_filtered vort_filt typevector_filt trials
disp('DONE.')
end